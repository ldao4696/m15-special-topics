...
for(i in 1:100) {
  ...
  training.indicies <- sample(seq_len(nrow(homes)), size = sample.size)
  training.data <- homes[training.indicies,]
  testing.data <- homes[-training.indicies,]
  ...
  assessment <- AssessFit(basic.fit, data=testing.data)
